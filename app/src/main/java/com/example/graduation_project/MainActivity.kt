...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportFragmentManager.beginTransaction().add(R.id.home_fragment_frame, homeFragment)
            .commit()
    }
            ...
            when (it.itemId) {
                R.id.nav_home -> {
                    ...
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.home_fragment_frame, homeFragment).commit()
                }
                R.id.nav_contest -> {
                    val contestFragment = ContestFragment()
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.home_fragment_frame, contestFragment).commit()
                }
                R.id.nav_write -> {
                    val writeFragment = WriteFragment()
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.home_fragment_frame, writeFragment).commit()
//                    val intent = Intent(this, WriteFragment::class.java)
//                    startActivity(intent)
                }
                R.id.nav_bookmark -> {
                    val bookmarkFragment = BookmarkFragment()
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.home_fragment_frame, bookmarkFragment).commit()
                }
                R.id.nav_profile -> {
                    val profileFragment = ProfileFragment()
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.home_fragment_frame, profileFragment).commit()
                }
            }
            true
        }
